...
export function ExerciseForm({
  ...
  const [machineBrand, setMachineBrand] = useState(initialValues?.machineBrand || '')
  const [targetRepRange, setTargetRepRange] = useState(initialValues?.targetRepRange || '')
  const [targetRepsInReserve, setTargetRepsInReserve] = useState(
    initialValues?.targetRepsInReserve?.toString() || ''
  )
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-[hsl(var(--color-muted-foreground))] mt-2">
          {mode === 'create'
            ? 'Add a new exercise with target parameters'
